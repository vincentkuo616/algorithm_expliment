...
def gompertzParameter_2(data):
    try:
        ...
        if len(data)<4:
            ...
            data4Compute = data[1:]
            ''' 產出連續值 '''
            data4ComputeCDF = [pk+'_CDF']
            for i in range(len(data4Compute)):
                if math.isnan(data4Compute[i]):
                    data4ComputeCDF.append(math.nan)
                else:
                    if i==0:    data4ComputeCDF.append(0+data4Compute[i])
                    else:   data4ComputeCDF.append(data4ComputeCDF[-1]+data4Compute[i])
