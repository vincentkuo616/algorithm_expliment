...
outputName4SF="//1349_productsOutput_All_4SF.xlsx"
saveFlag = 0
save4SFlag = 0
saveFlagLinear = 0
save4SFlagLinear = 0
...
def linearLoop(data):
    try:
        ...
        for i in dataArray:
            ...
            # 組分組資料(調整)
            yStageAdjusted=[i[0]+'_StageAdjusted']
            adjustedGroup=[]
            ...
            # 組警告訊號(調整 第二次才算)
            yWarningFlag=[i[0]+'_Warning']
            slopeTemp = ['N','N']
            if len(slope)>2:
                for u in range(2,len(slope)):
                    if slope[u]<0.1 and slope_inner[u]<slope_inner[u-1]:
                        slopeTemp.append('WarnWarn')
                    elif slope_inner[u]<slope_inner[u-1] and slope[u]<0.88:
            ...
            # 組警告訊號(第一次就看)
            yWarningSensitiveFlag=[i[0]+'_WarningSensitive']
    ...
    try:
        ...
        if len(dataX)<5:
            ...
            for i in range(0,len(group)-1):
                ...
                plt.plot(xfit,yfit)
                slope.append(model.coef_[0]/(testY[-1]/(group[i+1]-1)))
...
def groupRule(dataX,dataY):
    try:
        ...
        while group[-1]<len(dataX):
            ...
            if len(corrList)>0: ruleNum=group[-1]+5+rule(corrList)
            else: ruleNum=maxNum
        ...
        if times==3:
            for i in range(len(corrList)-3):
                ...
                if corrList[i]>corrList[i+1] and corrList[i+1]>corrList[i+2] and corrList[i+2]>corrList[i+3] and corrList[i]-corrList[i+1]>defOfDown and corrList[i+1]-corrList[i+2]>defOfDown and corrList[i+2]-corrList[i+3]>defOfDown:
                    index=i
                    break
                #elif corrList[i]-corrList[i+1]>0.5/(i+5) and corrList[i]-corrList[i+2]<0.017*(i+5):
                elif corrList[i]-corrList[i+1]>0.5/(i+5) and corrList[i]-corrList[i+2]<0.07:
                    index=i
                    break
                elif corrList[i]-corrList[i+1]>0.03*(i+5):
