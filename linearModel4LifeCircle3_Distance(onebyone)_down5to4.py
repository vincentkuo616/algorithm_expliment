...
times = 3
timesList = [3,4,5,6,7]
outputName4SFList=["//1349_productsOutput_All_4SF_03v14(5TO4).xlsx","//1349_productsOutput_All_4SF_Distance_03v14(5TO4).xlsx","//1349_productsOutput_All_4SF_Combine_03v14(5TO4).xlsx","//1349_productsOutput_All_4SF_Distance&Diff_03v14(5TO4).xlsx","//1349_productsOutput_All_4SF_Default_03v14(5TO4).xlsx"]
#timesList = [4,5,6,7]
...
def rule(corrList,dataListX,dataListY,groupValue,firstY):
    try:
        ...
        if times==2:
            for i in range(len(corrList)-2):
                ...
                if corrList[i]>corrList[i+1] and corrList[i+1]>corrList[i+2] and corrList[i]-corrList[i+1]>defOfDown and corrList[i+1]-corrList[i+2]>defOfDown:
                    index=i
                    break
                elif corrList[i]-corrList[i+1]>0.5/(i+4) and corrList[i]-corrList[i+2]<0.07:
                    index=i
                    break
                elif corrList[i]-corrList[i+1]>0.03*(i+4):
                    index=i
                    break
        #'''
        #DEFAULT
        ...
        if times==3: #方法一
            for i in range(len(corrList)-3):
                defOfDown=0.002
                defOfDown=(1.09-corrList[i])*0.5/(i+4)
        ...
        if times==5: #方法三
            for i in range(len(corrList)-3):
                ...
                if first<dataListY[i+4] and second<dataListY[i+5] and third<dataListY[i+6] and firstDiff*increment<secondDiff and secondDiff*increment<thirdDiff:
                    index=i
                    break
                #if dataListY[i+4]==dataListY[i+5] and dataListY[i+5]==dataListY[i+6] and model.coef_[0]>=0.2:
